 ...
 */
export const MANAGED_INCLUDES_TAG = '// ==== EX-Commander Required Includes ===='
export const MANAGED_HAL_DEVICES_TAG = '// ==== EX-Commander HAL Devices ===='
export const MANAGED_TRACK_MANAGER_TAG = '// ==== EX-Commander TrackManager ===='
export const MANAGED_TURNOUT_DEFAULTS_TAG = '// ==== EX-Commander Turnout Defaults ===='
...
export class ConfigEditorState {
    ...
    get automationPreview(): string {
        ...
        if (includes.length > 0) {
            ...
            sections.push('// These #includes are managed by EX-Commander.')
        ...
        if (this.generatedHalDevicesContent.trim()) {
            ...
            sections.push('// This HAL Devices block is managed by EX-Commander.')
    ...
    private _preserveDeviceHeader(newContent: string, existingContent: string): string {
        if (!hasDeviceHeader(newContent)) {
            ...
            if (hasDeviceHeader(existingContent)) {
                const headerEnd = existingContent.indexOf('// ==== End DCCEX-Commander Device Configuration ====')
                if (headerEnd !== -1) {
                    const headerBlock = existingContent.slice(0, headerEnd + '// ==== End DCCEX-Commander Device Configuration ===='.length + 1)
