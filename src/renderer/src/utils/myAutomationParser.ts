...
/** Marker embedded in the second line of every file generated by EX-Commander */
export const GENERATOR_HEADER_MARKER = '// DCCEX-Commander'

const GENERATOR_HEADER_RE = /^\/\/ DCCEX-Commander v([\d.]+)/m
const HEADER_BAR = '//' + ' ' + '='.repeat(77)

/**
 * Returns true if the text contains a DCCEX-Commander generator header.
...
export function buildGeneratorHeader(filename: string, appVersion: string): string {
    return [
        HEADER_BAR,
        `// DCCEX-Commander v${appVersion}`,
        `// This file (${filename}) is managed by EX-Commander — manual edits are preserved`,
