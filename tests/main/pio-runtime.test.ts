...
describe('resource paths', () => {
    ...
    it('keeps the writable core dir under the dcc-rail-commander home directory', () => {
        const dir = coreDir().replace(/\\/g, '/')
        expect(dir).toContain('/mock/home')
        expect(dir).toContain('dcc-rail-commander')
...
describe('seedRuntime()', () => {
    it('copies each bundled platform and package into the core dir', async () => {
        ...
        mockExistsSync.mockImplementation((p: unknown) => !String(p).replace(/\\/g, '/').includes('/dcc-rail-commander/platformio/'))
    ...
    it('writes the stamp so the next launch skips seeding', async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(
            expect.stringContaining('.dcc-rail-commander-toolchain'),
            'stamp-abc123',
            'utf-8',
        )
    })
    ...
    it('tolerates losing the rename race when a concurrent seed already installed the same entry', async () => {
        ...
        mockExistsSync.mockImplementation((p: unknown) => {
            ...
            if (s.includes('/dcc-rail-commander/platformio/') && s.endsWith('atmelavr') && !s.includes('.tmp-')) {
                targetCheckCount++
            ...
            return !s.includes('/dcc-rail-commander/platformio/')
        })
