/**
 * Shared Playwright fixtures for EX-Commander E2E tests.
 *
...
export const MOCK_CONFIG_H_COMPLETE = [
    '// config.h - Generated by EX-Commander',
    '',
    '#define IP_PORT 2560',
    '#define SCROLLMODE 1',
    '',
...
async function launchApp(): Promise<{ app: ElectronApplication; testDataDir: string }> {
    const testDataDir = mkdtempSync(join(tmpdir(), 'ex-commander-e2e-'))
...
async function launchIOExpanderApp(): Promise<{ app: ElectronApplication; testDataDir: string }> {
    const testDataDir = mkdtempSync(join(tmpdir(), 'ex-commander-e2e-iox-'))
...
async function launchRosterGroupedApp(): Promise<{ app: ElectronApplication; testDataDir: string }> {
    const testDataDir = mkdtempSync(join(tmpdir(), 'ex-commander-e2e-grp-'))
...
async function launchCsb1StackedApp(): Promise<{ app: ElectronApplication; testDataDir: string }> {
    const testDataDir = mkdtempSync(join(tmpdir(), 'ex-commander-e2e-csb1-'))
